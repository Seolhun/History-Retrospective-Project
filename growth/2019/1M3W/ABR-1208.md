...
1. 간단한 예로, 현재 `앱 > 대시보드 과정에서 필요한 데이터가 대시보드 랜더링 전에 로드되지 않음. 혹은 코드 구조상 중간에 업데이트 되는 현상이 발생 됨`
	- 즉, 대시보드에 필요한 데이터가 로딩 전, 관련 기반사항은 사전에 모두 충족한 조건이어야 하는데 그렇지가 않음.
		- 이는 추가로 아래 2번(Re-Rendering Issues)과 연관이 되어있므로,
	- Async Sequnece 문제가 복잡하게 엮여있기도 함. 이는 차후 To Do List에서 언급
2. Layout만을 사용하여 각각의 View에 관련된 Hierarchy를 분리하기 위한 로직이 Layout에 복잡하게 존재.
	- 로직의 복잡함(Layout)
	- 해당사항이 해당 View의 도착전에 Re-rendering을 시켜 속도를 늦추고, 하위 Container에게 영향을 미침
...
- 코드로 보면, 이전 Layout의 `React.createElement(children, property)`의 코드에서 property의 값이 먼저 변화가 된 후에, Router가 랜더링 되어야하지만, 그렇지 않아. 다시 Layout의 값이 변이되어 짧게나마 재랜더링이 되는 문제가 발생되던 것.
