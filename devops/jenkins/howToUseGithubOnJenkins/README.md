...
CI 툴을 이용한다는 것은 위 인용한 글 처럼, **손 쉽게 버그를 찾고 제거하기 위함일 것입니다.** 현재 지그재그도 Jenkins를 이용하여 CI를 운영하고 있으며, 최근에는 Storybook으로 UI를 테스트할 수 있는 시스템을 추가하였습니다. 앞으로도 지그재그는 자동화 CI 시스템을 확대할 계획입니다. 이번 포스트를 시작으로 CI 관련한 내용을 지속적으로 공유하고자 합니다.
...
<sub>
	CI(Integrate)와 CD(Delivery), CD(Deploy) Process의 차이.
...
위 그림을 통해 이번 포스트와 가장 연관된 부분은 `Code`와 `Build`, `Integrate` 부분입니다. 요즘 대부분이 Git(Github)을 통해 코드 버전관리를 하고 있으며, 이 부분이 `Code`, `Integrate`에 해당되는 내용입니다. 엄밀히 보면 `Build`, `Test`, `Release`, `Deploy` 과정이 Jenkins가 담당하는 부분입니다.

즉, Jenkins와 Git(Github)를 연결하는 방법에 대해 설명한다는 것은 `Code`, `Build`, `Integrate` 과정을 설명하는 것과 같습니다. 빌드 후 테스트까지 처리한다면 `Test` 과정도 추가가 되어 완벽한 CI를 구성 할수 있습니다. Github와 Jenkins의 연결방법을 이해하여 CI를 구성하기 위한 첫 과정을 이해하고자 첫 주제로 선정하였습니다.
...
- Jenkins를 구동시키기 위해서는 Jenkins가 설치되어있는 서버가 필요합니다. Jenkins 설치와 관련한 내용은 잘 정리되어 있는 글이 많기에 추가적으로 작성하지는 않았습니다. 설치가 필요하신 분들은 아래 링크를 통해 Jenkins를 설치하시기 바랍니다.
	...
	- `ufw enalbe`
		- `/github-web-hook/` Firewall에 막히는 문제가 발생된다. 추가적으로 Jenkins 서버에서 `web-hook`을 요청하는 인바운드가 포트가 막혀있을 경우 Timeout error가 발생된다.
...
## 5. 새로운 Item(Job) 만들기
- Item(Job)을 정의하는 곳입니다. SCM과의 연결 및 Build Trigger, Build, Test 등 
...
5. Github에 해당 Branch로 푸쉬할 수 있습니다.
...
- 핵심 키워드
	...
	- Stage
		- Stage block은 Pipeline의 상태/진행 상황을 시각화하거나 표시하기 위해 많은 플러그인에서 사용되는 전체 파이프 라인 (예 : "빌드", "테스트"및 "배포"단계)을 통해 수행되는 개념적으로 구분 된 작업의 하위 집합을 정의합니다.
	- Step
		- 근본적으로 한 단계는 젠킨스가 특정 시점 (또는 과정에서 "단계")에서해야 할 일을 알려줍니다. 예를 들어 쉘 명령을 실행하려면 sh 단계를 사용하십시오. sh 'make'. 플러그인이 Pipeline DSL을 확장하면 [1] 이는 일반적으로 플러그인이 새로운 단계를 구현했음을 의미합니다.
...
```groovy
node {
    git 'https://github.com/Seolhun/test-jenkins'
...
#### Private Repository를 가져오기 전 주의사항
- 인증 방법에 따라 System에서만 적용되는 곳이 있고, Job에만 적용되는 곳이 있습니다. Pipeline을 사용하여 System 값을 바로 사용하는거면 문제가 없을 것으로 판단되지만, FreeStyle처럼 UI로 Job을 생성하면 Credential 이용시 권한에 따라 선택할 수 없는 문제가 발생할 수 있습니다.
...
- 결론
	...
	- UserID/Password Credentials는 System config에서는 사용될 수 없고, 각 Job에서 사용될 수 있는 계층을 갖고 있는 것으로 보입니다.
